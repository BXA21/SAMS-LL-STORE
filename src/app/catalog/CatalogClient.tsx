...
interface CatalogClientProps {
  /*
   * Rendered on the server so the catalog is present in the HTML. Without
   * this the page shipped an empty grid and only filled in once the client
   * bundle had downloaded, hydrated and read its data — anyone whose
   * JavaScript was slow or blocked saw a permanently empty store.
   */
  initialProducts: Product[];
  initialCategories: Category[];
  /* `category` query parameter, resolved on the server. */
...
export default function CatalogClient({
  ...
  // Resolved from the server-supplied query parameter so the first paint
  // already reflects the requested category instead of flashing "all".
  const [selectedCategory, setSelectedCategory] = useState<string>(
    () => initialCategories.find((c) => c.slug === initialCategorySlug)?.id ?? 'all'
  );
   ...
   * catalog, so this only picks up admin edits held in this browser's local
   * storage — it must never clear what was rendered or show a loading state,
   * or the visible catalog would disappear on hydration.
   */
  useEffect(() => {
    let cancelled = false;
